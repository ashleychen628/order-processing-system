...
public class OrderService {
    ...
    public OrderDetails cancelOrderById(String orderNumber) throws OrderIdNotFoundException {
        ...
        if (orderDetails.isPresent()) {
            ...
            LOGGER.error("Order number " + orderNumber + " is not found");
            throw new OrderIdNotFoundException(HttpStatus.NOT_FOUND, "Order number " + orderNumber + " is not found. Provide Existing Order Number");
        }
    }

    @Transactional
    public OrderDetails updateOrderStatus(OrderDetails requestedOrderDetails) throws OrderIdNotFoundException, EnumNotPresentException {
        ...
        try {
            ...
            throw new EnumNotPresentException(HttpStatus.BAD_REQUEST, requestedOrderDetails.getOrderStatus() + " is not found. Provide correct order status.");
        }
        ...
        if (orderDetails.isPresent()) {
            ...
            if (requestedOrderStatus.name().equals("DECLINED")) {
                LOGGER.warn("Order number " + updatingOrderNumber + " is " + requestedOrderStatus.name());
                orderDetails.get().getShipping().setShippingStatus(ShippingStatus.CANCELLED);
            }
            if (requestedOrderStatus.name().equals("DELIVERED")) {
                LOGGER.info("Order number " + updatingOrderNumber + " is " + requestedOrderStatus.name());
            ...
            if (requestedOrderStatus.name().equals("SHIPPED")) {
                ...
                LOGGER.info("Order number " + updatingOrderNumber + " is " + requestedOrderStatus.name() + " and " + "Shipping status is " + ShippingStatus.SHIPPED);
            }
    ...
    @Transactional
    public OrderDetails getOrderByCustomerIDAndOrderID(long customerId, String orderNumber) throws CustomerIdNotFoundException, NoMatchException {
        LOGGER.info("Searching order by customerId: " + customerId + " and orderNumber: " + orderNumber);

        Optional<OrderDetails> existingOrder = orderRepository.findByCustomerIdAndOrderNumber(customerId, orderNumber);
        if (existingOrder.isPresent()) {
            LOGGER.info("The Order submitted by " + customerId + " is " + existingOrder.get().getOrderStatus() + "");
            return existingOrder.get();
        } else {
            LOGGER.error("customer Id and order id not found");
            throw new NoMatchException(HttpStatus.NOT_FOUND, customerId + " and " + orderNumber + " not exist");
        }
    }
